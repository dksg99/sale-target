...
> **Trạng thái:** Bản mô tả sản phẩm hiện hành + định hướng
> **Phiên bản tài liệu:** 1.0
> **Ngày:** 2026-07-20
> **Chủ sở hữu sản phẩm:** (điền)
...
**Sale Target** là ứng dụng web một trang (single-page app) giúp đội ngũ kinh doanh **lập, theo dõi và cập nhật kế hoạch bán hàng** theo từng tháng, phân rã theo Team → Miền → Phụ trách (PS) → Khách hàng → Sản phẩm.

Ứng dụng cho phép:
- Nhập/sửa số lượng kế hoạch & thực hiện trực tiếp trên bảng (spreadsheet-like) theo 12 tháng của năm tài chính.
...
**Định vị một câu:** Một công cụ tập trung, thời gian thực, thay thế bảng tính rời rạc — để mọi cấp quản lý kinh doanh nhìn cùng một con số kế hoạch/thực hiện và chủ động điều chỉnh.
...
- Kế hoạch bán hàng trước đây chạy trên nền **Google Apps Script + Google Sheets** (mật khẩu hash SHA-256 được kế thừa lại từ hệ cũ để không phải reset toàn bộ tài khoản).
- Dữ liệu phân tán, khó phân quyền theo vai trò, khó tổng hợp nhiều chiều, hiệu năng kém khi dữ liệu lớn.

### 2.2 Vấn đề cần giải quyết
1. **Một nguồn sự thật (single source of truth):** nhiều cấp (admin, quản lý team, quản lý ngành hàng, quản lý miền, PS) cần cùng nhìn một bộ số nhất quán.
...
4. **Tổng hợp đa chiều:** theo PS/Khách hàng và theo Sản phẩm/Miền, kèm chỉ số quota & doanh thu.
5. **Hiệu năng ở quy mô ~20.000 dòng:** tải nhanh, sửa mượt.

### 2.3 Giải pháp đã chọn
Chuyển sang **Supabase** (Postgres + Edge Functions) làm backend, frontend tĩnh (React qua CDN) host trên **GitHub Pages**. Toàn bộ nghiệp vụ đi qua một Edge Function API duy nhất với token có chữ ký HMAC.
...
Mỗi bản ghi kế hoạch gắn với một **Team** (`bu`). Các team hiện có: **CHCS, CTTM, THNK** (và TEST cho kiểm thử; CTCH tạm ẩn cho tới khi có dữ liệu thật).
...
- `product_manager` có thể phụ trách **nhiều ngành hàng**, ngăn cách bằng dấu phẩy trong `scope`; chưa gán ngành → không thấy dữ liệu.
...
- **Area Manager:** theo dõi miền phụ trách.
...
- **Sửa trực tiếp từng ô** (chỉ admin/ps). Ô đang sửa có viền nhấn; ô chưa lưu (pending) tô màu hổ phách.
...
1. **Một sản phẩm có thể có nhiều dòng** nếu **đơn giá khác nhau** → **không dedup** theo sản phẩm.
2. **Đẩy `sl_thuc_hien` phải khớp theo PS**: một khách hàng có thể do nhiều PS phụ trách; import thực hiện phải khớp đúng dòng có PS tương ứng (pipeline ngoài, không qua UI).
3. **Năm tài chính**: 12 tháng từ **04/2026 → 03/2027**.
...
- Mật khẩu lưu dạng **SHA-256** (kế thừa hệ cũ). *Ghi chú rủi ro: SHA-256 không salt — xem §12.*
...
### 12.1 Rủi ro
- **Mật khẩu SHA-256 không salt** (kế thừa hệ cũ) → yếu trước tấn công dò bảng băm. Cân nhắc chuyển bcrypt/argon2.
...
*Tài liệu mô tả theo trạng thái mã nguồn tại nhánh `main` ngày 2026-07-20. Các mục đánh dấu "(điền)" hoặc "cần chốt" chờ chủ sở hữu sản phẩm bổ sung.*
